...
module.exports = ({width = 600, height = 300, data, series =[], colorGenerator, autoColorOptions,yAxisLabel, ...props} = {}) => {
    ...
    const detict=(thing)=>seriesArray.map(({color, ...serie} = {}, i) => <Bar key={`bar-${i}`} name={thing} fill={COLORS[i]} {...serie}/>)

    return (<BarChart key={key} autoColorOptions={autoColorOptions} width={width} height={height} data={data}>

       {yAxisLabel?detict(yAxisLabel):detict(false)}
